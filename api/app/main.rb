...
class MAPTheAPI < Sinatra::Base
  ...
  configure do
    ...
    # TEMPORARY: Add URLs to existing data.  We can remove this eventually.
    AspaceDB.open do |aspace_db|
      DB.open do |map_db|
        map_db[:file_issue_item].each do |fi|
          updates = {}
          ...
          unless updates.empty?
            map_db[:file_issue_item].filter(:id => fi[:id]).update(updates)
          end
        end
      end
    end
    ...
    Ctx.open do
      ...
      unless Users.user_exists?('admin')
        admin_id = Users.create_admin_user('admin', 'Admin User', 'admin@example.com', '__system_bootstrap')
